...
<template>
  <header class="w-full bg-white font-sans shadow-md">
    ...
    <nav class="flex items-center justify-between p-4 lg:px-8 max-w-7xl mx-auto">
      <div class="flex items-center flex-shrink-0">
        <RouterLink to="/" @click="handleLinkClick('/')" class="cursor-pointer">
          <img :src="hfn_logo" alt="HFN Logo" class="w-30 h-auto mr-3 flex-shrink-0" />
        </RouterLink>
      </div>
      ...
      <div class="hidden lg:flex items-center space-x-2 xl:space-x-4">
        <div v-for="link in navLinks" :key="link.title" class="relative group">
          <RouterLink :to="link.path" @click="handleLinkClick(link.path)"
    ...
    <div :class="[
      ...
      <div :style="{ backgroundColor: DARK_GREEN }" class="flex justify-between items-center p-4 text-white">
      ...
      <div class="flex flex-col p-4 space-y-2">
        <div v-for="link in navLinks" :key="link.title">
          <div v-if="!link.hasDropdown"
            ...
            <RouterLink :to="link.path" class="block w-full" @click="handleLinkClick(link.path)">
              {{ link.title }}
            </RouterLink>
          </div>
          ...
          <div v-else>
            ...
            <div v-if="openDropdown === link.title"
              ...
              <RouterLink v-for="item in link.dropdownItems" :key="item.title" :to="item.path"
                class="block py-2 text-sm text-gray-700 hover:text-[#004d33]" @click="handleLinkClick(item.path)">
                {{ item.title }}
              </RouterLink>
            </div>
          </div>
