...
<template>
  ...
  >
    <div class="max-w-md w-full">
      ...
      <div v-if="paymentInfo" class="bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div class="bg-[#004d33] p-8 text-white text-center">
          <p
            class="text-green-200 text-xs uppercase tracking-widest mb-1 font-semibold"
          >
            Amount Due
          </p>
        ...
        <div class="p-8 space-y-5">
          ...
          >
            ...
            <div
              class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-transparent group-hover:border-green-200 transition-all"
            >
          ...
          <div
            class="mt-6 p-4 bg-orange-50 rounded-2xl border-2 border-dashed border-[#E87A18]/30"
          >
            <p class="text-xs text-[#E87A18] font-bold uppercase mb-1">
              Payment Reference
            </p>
            <div class="flex justify-between items-center">
              <span class="text-xl font-mono font-black text-[#E87A18]">{{
                accountDetails.referenceCode
              }}</span>
              <button
                @click="copyToClipboard(accountDetails.referenceCode)"
                class="text-[#E87A18] font-bold text-sm underline"
              >
                Copy
              </button>
            </div>
            <p
              class="text-[10px] text-gray-500 mt-2 uppercase italic tracking-tighter"
            >
              * MUST include this in your transfer description
            </p>
          </div>
        </div>
    ...
    <transition name="pop">
      <div
        v-if="copied"
        class="fixed bottom-8 bg-gray-900 text-white px-6 py-3 rounded-full shadow-xl text-sm font-bold z-50"
      >
        Copied to clipboard!
      </div>
    </transition>
