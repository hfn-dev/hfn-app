    ...
    <div class="flex flex-grow">
      ...
      <main class="flex-grow p-4 md:p-8 lg:p-12">
        ...
        <div v-else-if="userEnrollments.length === 0" class="text-center py-12">
          ...
          <button @click="router.push('/user/courses')"
        ...
        <div v-else-if="activeCourses.length > 0" class="mb-12">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl">
            <div v-for="enrollment in paginatedActiveCourses" :key="enrollment.id"
              ...
              <div class="relative w-full h-36 bg-gray-100">
                ...
                <div class="absolute bottom-0 left-4 transform translate-y-1/2 p-1 bg-white rounded-full shadow-lg">
                  <div
              ...
              <div class="p-4 pt-8 flex flex-col flex-grow">
                ...
                <button @click="continueLearning(enrollment)"
          ...
          <div v-if="totalActivePages > 1" class="flex justify-center items-center mt-8 space-x-4">
            <button @click="goToActivePage(activePage - 1)" :disabled="activePage === 1"
              class="text-gray-500 disabled:opacity-50 hover:text-gray-700 p-2 rounded-full transition">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 transform rotate-180" viewBox="0 0 24 24"
            ...
            <button @click="goToActivePage(activePage + 1)" :disabled="activePage === totalActivePages"
              class="text-gray-500 disabled:opacity-50 hover:text-gray-700 p-2 rounded-full transition">
        ...
        <div v-if="completedCourses.length > 0">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl">
            <div v-for="enrollment in paginatedCompletedCourses" :key="enrollment.id"
              class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transform transition-all duration-300 hover:shadow-xl">
              <div class="relative w-full h-36 bg-gray-100">
                <img v-if="enrollment.course?.thumbnail" :src="enrollment.course.thumbnail"
                  :alt="enrollment.course.title" class="w-full h-full object-cover" />
...
Completed on {{ enrollment.completed_at ? new Date(enrollment.completed_at).toLocaleDateString() : '—' }}
                ...
                <button @click="reviewCourse(enrollment)"
                  class="mt-auto py-2 rounded-lg font-semibold transition duration-200" :style="{
          ...
          <div v-if="totalCompletedPages > 1" class="flex justify-center items-center mt-8 space-x-4">
