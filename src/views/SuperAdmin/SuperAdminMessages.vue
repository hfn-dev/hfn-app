...
const tabs = ["Direct Messages", "Groups", "Messages", "Connections"];
...
const loadConnections = async () => {
  try {
    ...
    const res = await messagingApi.listConnections();
    connections.value = res.accepted || [];
...
const acceptRequest = async (userId) => {
  try {
    await messagingApi.acceptConnectionRequest(userId);
    const acceptedUser = pendingRequests.value.find((u) => u.id === userId);
    connections.value.push(acceptedUser);
...
const rejectRequest = async (userId) => {
  try {
    await messagingApi.declineConnectionRequest(userId);
    pendingRequests.value = pendingRequests.value.filter(
      (u) => u.id !== userId
    );
  } catch (err) {
...
<template>
  <div class="flex min-h-screen bg-gray-50">
    ...
    <main class="flex-1 p-8 overflow-auto md:p-8 lg:p-12">
        ...
        >
          ...
          <div v-if="currentTab === 'Direct Messages'" class="space-y-1">
            <div class="relative mb-3">
              ...
              <div
                v-if="searchQuery && !userSearchResults.length"
      ...
      <div v-if="currentTab === 'Connections'" class="max-w-5xl space-y-6">
  <div v-if="loadingConnections" class="text-center py-12 text-gray-400">
    Loading connections…
  </div>

  <div v-if="pendingRequests.length">
    <h3 class="text-lg font-semibold mb-2">Pending Requests</h3>
    <div v-for="user in pendingRequests" :key="user.id" class="flex justify-between items-center bg-white p-4 rounded-lg shadow">
      <div>
        <p class="font-medium">{{ user.full_name }}</p>
        <p class="text-sm text-gray-500">{{ user.email }}</p>
      </div>
      <div class="flex space-x-2">
        <button @click="acceptRequest(user.id)" class="px-3 py-1 bg-green-600 text-white rounded-lg">Accept</button>
        <button @click="rejectRequest(user.id)" class="px-3 py-1 bg-red-600 text-white rounded-lg">Reject</button>
      </div>
    </div>
  </div>

  <div v-if="connections.length">
    <h3 class="text-lg font-semibold mb-2 mt-6">Your Connections</h3>
    <div v-for="user in connections" :key="user.id" class="bg-white p-4 rounded-lg shadow">
      <p class="font-medium">{{ user.full_name }}</p>
      <p class="text-sm text-gray-500">{{ user.email }}</p>
    </div>
  </div>

  <p v-if="!loadingConnections && !connections.length && !pendingRequests.length" class="text-center text-gray-400 py-12">
    No connections yet
  </p>
</div>
